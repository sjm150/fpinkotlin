...
fun <A> Stream<A>.take(n: Int): Stream<A> =
    Stream.unfold(this to n) { (s, x) ->
        if (x > 0 && s is Cons) Some(s.head() to (s.tail() to x - 1))
        else None
    }
...
fun <A> Stream<A>.takeWhile(p: (A) -> Boolean): Stream<A> =
    Stream.unfold(this) {
        when (it) {
            is Cons ->
                if (p(it.head())) Some(it.head() to it.tail())
                else None
            is Empty -> None
        }
    }
...
fun <A, B, C> Stream<A>.zipWith(
    ...
    Stream.unfold(this to that) { (sa, sb) ->
        if (sa is Cons && sb is Cons) Some(f(sa.head(), sb.head()) to (sa.tail() to sb.tail()))
        else None
    }
...
fun <A, B> Stream<A>.zipAll(
    ...
    fun <T> Stream<T>.pair(): Pair<Option<T>, Stream<T>> = when (this) {
        is Cons -> Some(head()) to tail()
        is Empty -> None to empty()
    }

    return Stream.unfold(this to that) { (sa, sb) ->
        if (sa is Empty && sb is Empty) {
            None
        } else {
            sa.pair().let { (ah, at) ->
                sb.pair().let { (bh, bt) ->
